    ...
    if (bolaActiva[i]) {
      ...
      if (bolaEnPas[i]) {
        ...
        if (abs(dx) < 1 && abs(dy) < 1) {
          if (bolaEnCaixa[i]) {
            // Animació cap a caixa completada
            bolaActiva[i] = false;
            puntuacio++;
            bolaActualEsq = !bolaActualEsq;
          } else {
            // Animació de pas completada
            bolaY[i] = -10;
            bolaEnPas[i] = false;
            bolaPassada = true;
          }
...
function keyPressed() {
  for (let i = 0; i < numBoles; i++) {
    if (bolaActiva[i] && !bolaEnPas[i]) {
      ...
      // Tecles a, s i fletxes amb animació cap a caixa
      ...
      // Tecla g manté comportament original
      if (key === 'g') {
      ...
      // Tecla f per eliminar bola passada
